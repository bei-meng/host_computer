start:
    ...
    addi col_bank_end, zero, 4                              # 截止读的列bank号
    ...
    addi count_max, zero, 128                               # 设置计数器最大值

    addi pq, zero,0
    # ---------------------------------------------------------------------------------------------------
    addi row_index_start_addr, zero, 0x000                  # 初始化寄存器，存放din_ram中的第一个行bank起始读的行index的addr
    addi row_index_end_addr, zero, 0x002                    # 初始化寄存器，存放din_ram中的第一个行bank截止读的行index的addr
...
loop2:
    addi col_index_start_addr, zero, 0x004                  # 初始化寄存器，存放din_ram中的第一个列bank起始读的列index的addr
    addi col_index_end_addr, zero, 0x006                    # 初始化寄存器，存放din_ram中的第一个列bank起始读的列index的addr
